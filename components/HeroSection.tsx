  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center animate-fade-in-up">
        ...
        {/* Call and WhatsApp buttons - high converting & compact */}
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center mt-2">
          {/* Call button with phone number */}  
          <a
            id="hero-call"
            href={CONTACT_LINKS.phone}
            className="group w-full max-w-sm sm:w-auto px-6 py-3.5 rounded-full bg-gradient-to-r from-amber-600 to-amber-700 text-white font-bold hover:from-amber-500 hover:to-amber-600 hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-600/30 transition-all duration-300 inline-flex items-center justify-center gap-2 text-base shadow-lg border border-amber-500/50"
            aria-label={`Call ${BUSINESS_INFO.phone}`}
          >
            <svg className="w-5 h-5 group-hover:animate-bounce drop-shadow-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* WhatsApp button (Green + Animating) */}
          <a
            id="hero-whatsapp"
            href={CONTACT_LINKS.whatsapp}
            target="_blank"
            rel="noopener noreferrer"
            className="group relative w-full max-w-sm sm:w-auto px-6 py-3.5 rounded-full bg-gradient-to-r from-[#25D366] to-[#128C7E] text-white font-bold hover:from-[#20bd5a] hover:to-[#0f7a6e] hover:-translate-y-1 hover:shadow-xl hover:shadow-green-500/40 transition-all duration-300 inline-flex items-center justify-center gap-2 text-base shadow-lg border border-green-400/50 animate-pulse-ring"
